<?php
// Tính canAddToCart nếu chưa được set từ trang gọi
if (!isset($canAddToCart)) {
    $canAddToCart = true;
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }
    if (isset($_SESSION['user_role_name'])) {
        $roleLower = strtolower($_SESSION['user_role_name']);
...
// Đường dẫn tới giỏ hàng — tương đối so với trang gọi
if (!isset($cartPath)) {
    $cartPath = isset($basePath) ? $basePath . 'pages/cart/index.php' : 'pages/cart/index.php';
}
?>
<!-- Product Customization Side Modal -->
                    ...
                    <form id="modal-product-form">
                        ...
                        <?php if ($canAddToCart): ?>
                            <div class="product-actions" style="position: sticky; bottom: 0; background-color: var(--white); padding: 16px 20px; border-top: 1px solid var(--border-color); box-sizing: border-box; display: flex; gap: 10px; margin-top: auto;">
                                ...
                                <a href="<?php echo htmlspecialchars($cartPath); ?>" class="btn-view-cart" style="text-decoration: none; width: auto; flex: 1; display: flex; align-items: center; justify-content: center;">
                                    Xem giỏ hàng
                                </a>
                            </div>
                        <?php else: ?>
