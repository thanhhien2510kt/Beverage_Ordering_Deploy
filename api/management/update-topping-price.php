<?php
/**
 * Management Update Topping Price API
 * Update topping price (Admin only)
 */

header('Content-Type: application/json');
require_once '../../functions.php';

if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

$response = ['success' => false, 'message' => ''];

try {

    if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
        throw new Exception('Bạn cần đăng nhập để thực hiện thao tác này');
    }


    $userRole = $_SESSION['user_role_name'] ?? '';
    if ($userRole !== 'Admin') {
        throw new Exception('Chỉ Admin mới có quyền điều chỉnh giá topping');
    }


    $toppingId = isset($_POST['topping_id']) ? (int)$_POST['topping_id'] : 0;
    $newPrice = isset($_POST['price']) ? trim($_POST['price']) : '';


    if (!$toppingId) {
        throw new Exception('Mã topping không hợp lệ');
    }

    if (empty($newPrice) || !is_numeric($newPrice) || $newPrice < 0) {
        throw new Exception('Giá thêm không hợp lệ');
    }


    $pdo = getDBConnection();


    $sql = "UPDATE Option_Value SET GiaThem = ? WHERE MaOptionValue = ? AND MaOptionGroup = 3";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$newPrice, $toppingId]);

    if ($stmt->rowCount() === 0) {
        throw new Exception('Không tìm thấy topping hoặc giá không thay đổi');
    }

    $response = [
        'success' => true,
        'message' => 'Cập nhật giá topping thành công'
    ];

} catch (Exception $e) {
    $response['message'] = $e->getMessage();
} catch (PDOException $e) {
    error_log("Database error in update topping price: " . $e->getMessage());
    $response['message'] = 'Có lỗi xảy ra. Vui lòng thử lại sau.';
}

echo json_encode($response, JSON_UNESCAPED_UNICODE);
exit;
